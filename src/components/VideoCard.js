...
const VideoCard = ({ videoData, channelsList = [] }) => {
    ...
    return (
        <div style={(isMenuOpen ? {width: "380px"} : {width: "330px"})}>
            ...
            <div className="flex py-2 cursor-pointer">
                <div className="relative">
                    <div className={"h-12 w-12 rounded-full p-[3px] flex " + (snippet?.liveBroadcastContent === "live" ? "border-[2px] border-red-600" : "")}>
                        <img className="cursor-pointer h-full w-full rounded-full" alt="Channel Image" src={channelData[0]?.snippet?.thumbnails?.default?.url || snippet?.thumbnails?.high?.url} />
                        {
